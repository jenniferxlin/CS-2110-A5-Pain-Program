...
public class DrawingCanvas extends JPanel implements MouseListener, MouseMotionListener  {
    ...
    @Override public void paintComponent(Graphics g) {
        ...
        if (pointGiven == true) {
        	...
        	drawLine(g2d, linePoint, mousePos, foreColor, toolSize);
            repaint();
        }
        ...
        if (centerGiven == true) {
        	...
        	double r = Math.sqrt(Math.pow((center.x - mousePos.x), 2) + 
        			Math.pow((center.y - mousePos.y), 2));
        	Shape circle = new Ellipse2D.Double(center.x - r, center.y - r, 
        			2*r, 2*r);
        	g2d.draw(circle);
            repaint();
        }
    ...
    @Override public void mousePressed(MouseEvent e) {
        ...
        if (activeTool == Tool.PENCIL) {
            ...
            g2d.setColor(foreColor);
            g2d.setStroke(new BasicStroke(toolSize));
            Shape rect= new Rectangle2D.Double(mousePos.x - half, 
            		mousePos.y - half, toolSize, toolSize);
            g2d.draw(rect);
            g2d.fill(rect);
            ...
            // If one has already been made, this is the second mouse press;
            // draw the circle.
            if (centerGiven == false) {
            	center = mousePos;
            	centerGiven = true;
            } else if (centerGiven == true) {
            	Color foreColor = getForeColor();
            	g2d.setColor(foreColor);
            	g2d.setStroke(new BasicStroke(toolSize));
            	double r = Math.sqrt(Math.pow((center.x - mousePos.x), 2) + 
            ...
            int squarePercent = (toolSize*toolSize)/10;
            int i = 0;
            while (i < squarePercent) {
            	Shape pixel = new Rectangle2D.Double(toolSize*gen.nextDouble() + 
            			(mousePos.x - half), toolSize*gen.nextDouble() + 
            			(mousePos.y - half), 1, 1);
            	g2d.draw(pixel);
            	i = i+1;
            }
    ...
    @Override public void mouseDragged(MouseEvent e) {
        ...
        if (activeTool == Tool.PENCIL) {
        	...
        	Color foreColor = getForeColor();
        	drawLine(g2d, mousePosPrev, mousePos, foreColor, toolSize);
            repaint();
        }
        else if (activeTool == Tool.ERASER) {
            ...
            // mouse position mousePosPrev to position mousePos.
        	Color backColor = getBackColor();
        	drawLine(g2d, mousePosPrev, mousePos, backColor, toolSize);
        	repaint();
        }
        else if (activeTool == Tool.COLOR_PICKER) {
            // Nothing to do here.
        	...
        	Color foreColor = getForeColor();
            g2d.setColor(foreColor);
            double x = toolSize*gen.nextDouble() + (mousePos.x - half);
            double y = toolSize*gen.nextDouble() + (mousePos.y - half);
            
            int squarePercent = (toolSize*toolSize)/10;
            int i = 0;
            while (i < squarePercent) {
            	Shape pixel = new Rectangle2D.Double(x, y, 2, 2);
            	g2d.setStroke(new BasicStroke(toolSize));
            	g2d.draw(pixel);
            	i = i+1;
            }
    ...
    /**Draws a line between two mouse positions in the given color and stroke
     * on the image. */
    private void drawLine(Graphics2D g2d, Point2D.Double prevPos, Point2D.Double
    		currentPos, Color c, int strokeSize) {
    	g2d.setColor(c);
    	g2d.setStroke(new BasicStroke(strokeSize));
    	g2d.draw(new Line2D.Double((int) prevPos.x, (int) prevPos.y, 
